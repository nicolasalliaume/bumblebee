	...
	with open(os.path.join(dirname, filename), "r") as source:
		with open(os.path.join("new", dirname, filename), "w") as target:
			...
			if __filter__(filename):
				...
				#
				# this is an example:
				target_content = target_content.replace("Steve Jobs", "The GREAT Steve Jobs")
				target_content = target_content.replace("Madonna", "Madonna (sounds MA-DDDO-NA")
				target_content = target_content.replace("Facebook", "some popular social network")
				target_content = target_content.replace("The Who", "The Why")
				target_content = target_content.replace("Jodie Foster", "Bumblebee's creator wife")
				target_content = target_content.replace("Ryan", "Rihanna")
				#######################################
